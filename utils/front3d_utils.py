...
from utils.front3d_config import FRONT3D_CONFIG, FRONT3D_DATA
import pandas as pd
...
class_mapping = pd.read_csv(front3d_config.class_mapping_file).drop(['Unnamed: 0'], axis=1)
...
def correct_flipped_objects(obj_points, transform_matrix, model_path, voxels=None, sampled_points=None, flipped_objects_in_sunrgbd=[]):
    ...
    if model_path.split('/')[-1] in flipped_objects_in_3dfront:
...
def read3dfrontframe(config, image_name=None, image_id=None):
    ...
    img_info['manhattan_layout'] = loadmat(os.path.join(front3d_config.data_root, '3dlayout', str(image_id) + '.mat'))['manhattan_layout'].T
    ...
    # use updated R_tilt
    R_tilt = loadmat(os.path.join(front3d_config.data_root, 'updated_rtilt', str(image_id) + '.mat'))['r_tilt']
    ...
    data_frame = FRONT3DData(img_info['K'], img_info['R_ex'], img_info['R_tilt'], img_info['bdb2d'], img_info['bdb3d'],
...
def process_3dfront_frame(sample, flip=False):
    '''
    ...
    if not flip:
        ...
        frame = FRONT3D_DATA(cam_K, cam_R, sample.scene_type, sample.imgrgb, sample.imgdepth, layout_3D, sample.sequence_id,
        ...
        frame = FRONT3D_DATA(cam_K_flip, cam_R_flip, sample.scene_type, rgb_img, depth_map, layout_3D_flip,
