...
class ResNet2TSDF(nn.Module):
    
    def __init__(self, opt):
        super(ResNet2TSDF, self).__init__()
        ...
        self.convt3 = PVQVAEResnetBlock(in_channels=in_c, out_channels=in_c//2, temb_channels=0, dropout=0.1)
        
        self.attn3 = AttnBlock(in_c//2)
        self.up3 = Upsample(in_c, True)
        self.norm_out = Normalize(in_c//2)
        self.conv_out = torch.nn.Conv3d(in_c//2, 1, 1)
    ...
    def forward(self, x):
        ...
        temb=None
        x = self.convt_layers(x)  #  torch.Size([1, 32, 64, 64, 64])

        x = self.convt3(x) #  torch.Size([1, 32, 64, 64, 64])
        ...
        return x # torch.Size([1, 1, 64, 64, 64])