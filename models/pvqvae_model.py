## Source: https://github.com/yccyenchicheng/AutoSDF 
## Model for P-VQ_VAE training
...
class PVQVAEModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        print('nC ', nC)
        print('cube size ', self.cube_size)
        self.ncubes_per_dim = nC // self.cube_size
        print('ncubes per dim ', self.ncubes_per_dim)
    ...
    def forward(self):
        ...
        #print('self zq_cubes ', self.zq_cubes.shape)  # torch.Size([1024, 256, 1, 1, 1])
        #print('self qloss shape ', self.qloss.shape) #scalar 
        ...
        #print('zq fold to voxel ', self.zq_voxels.shape) # torch.Size([2, 256, 8, 8, 8])
        self.x_recon = self.vqvae.decode(self.zq_voxels)
        #print('vqvae decoder ', self.x_recon.shape) #  torch.Size([2, 1, 64, 64, 64])
    ...
    def test_iou(self, data, thres=0.0):
        ...
        self.vqvae.eval()
        self.inference(data, should_render=True) #true so that we get pix3d1,pix3d2, etc ... 
        self.vqvae.train()
    ...
    def backward(self):
        ...
        #print('backward x ', self.x.shape)
        #print('backward x recon ', self.x_recon.shape)
