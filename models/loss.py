...
from net_utils.libs import get_bdb_form_from_corners, \
    ...
    get_bdb_2d_result, physical_violation, recover_points_to_obj_sys, get_bdb_3d_result
import numpy as np
...
def get_point_loss(points_in_world_sys, cam_R, cam_K, depth_maps, bdb3D_form, split, obj_masks, mask_status):
    ...
    for scene_id, obj_interval in enumerate(split):
        ...
        for loc_id, obj_id in enumerate(range(*obj_interval)):
            ...
            #n_objects += 1
    return  torch.tensor(0) #depth_loss/n_objects if n_objects > 0 else torch.tensor(0.).to(device)
...
def get_layout_bdb(bins_tensor,pitch_cls_result,pitch_reg_result,lo_centroid_reg,lo_coeffs_reg):
    ...
    lo_centroid = torch.tensor(avg_layout['layout_centroid_avg']).float().cuda() + lo_centroid_reg
    lo_coeffs=torch.tensor(avg_layout['layout_coeffs_avg']).float().cuda()+lo_coeffs_reg
...
class PoseLoss(BaseLoss):
    def __call__(self, est_data, gt_data, bins_tensor):
        ...
        lo_bdb3D_result = get_layout_bdb(bins_tensor2, est_data['pitch_cls_result'], est_data['pitch_reg_result'],
...
class JointLoss(BaseLoss):
    def __call__(self, est_data, gt_data, bins_tensor, layout_results):
        ...
        return {'phy_loss':phy_loss, 
        #'bdb2D_loss':bdb2D_loss,
