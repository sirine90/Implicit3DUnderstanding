...
class Trainer(BaseTrainer):
    ...
    def to_device(self, data):
        ...
        if self.cfg.config[self.cfg.config['mode']]['phase'] in ['object_detection', 'joint']:
            ...
            patch_jid = data['boxes_batch']['patch_jid'] #.to(device)
            patch_mask_jid = data['boxes_batch']['patch_mask_jid'] #.to(device)
            ...
            jid = data['boxes_batch']['jid'] #.to(device)
            ...
            object_input = {'patch':patch, 'patch_jid': patch_jid, 'patch_mask_jid': patch_mask_jid, 'jid':jid, 'g_features':g_features, 'size_reg':size_reg, 'size_cls':size_cls,
        ...
        if self.cfg.config[self.cfg.config['mode']]['phase'] in ['joint']:
            ...
            joint_input = {'bdb3D':bdb3D, 'K':K, 'depth_maps':depth_maps, 'cam_R_gt':cam_R_gt,
                           ...
                           #'bdb2D_from_3D_gt':bdb2D_from_3D_gt, 
                           'bdb2D_pos':bdb2D_pos}
