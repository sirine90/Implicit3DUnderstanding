...
from utils.qual_util import crop_square
...
class FUTUREDataset(BaseDataset):

    def initialize(self, opt, phase='train', cat='all'):
        self.opt = opt
        self.max_dataset_size = opt.max_dataset_size

        with open(f'{dataroot}/FUTURE/info.json') as f:
        ...
        for c in all_cats:
            ...
            with open(f'{dataroot}/FUTURE/filelists/{synset}_{phase}.lst') as f:
                model_list_s = []
                for l in f.readlines():
                    model_id = l.rstrip('\n')
                    
                    path = f'{dataroot}/FUTURE/SDF_v1_64/{synset}/{model_id}/ori_sample.h5'
...
class FUTURENetCodeDataset(BaseDataset):
        ...
        for c in all_cats:
            ...
            with open(f'{dataroot}/FUTURE/filelists/{synset}_{phase}.lst') as f:
                ...
                for l in f.readlines():
                    ...
                    # path = f'{dataroot}/FUTURE/SDF_v1_64/{synset}/{model_id}/ori_sample_grid.h5'
    ...
    def name(self):
        return 'FUTURENetCodeDataset'


class FUTURENetImgDataset(BaseDataset):
    ...
    def initialize(self, opt, phase='train', cat='all'):
        ...
        with open(f'{dataroot}/FUTURENet/info.json') as f:
            self.info = json.load(f)

        # code_setting = f'{opt.vq_model}-{opt.vq_dset}-{opt.vq_cat}-T{opt.trunc_thres}'
        code_setting = get_code_setting(opt)
        print('CODE SETTING ', code_setting)
        self.code_dir = f'{dataroot}/extracted_code/{code_setting}'
        print(self.code_dir)
        assert os.path.exists(self.code_dir), f'{self.code_dir} should exist.'
        ...
        for c in cats:
            ...
            with open(f'{dataroot}/FUTURE/filelists/{synset}_{phase}.lst') as f:
                ...
                for l in f.readlines():
                    ...
                    render_img_dir = os.path.join(dataroot, 'FUTURE', 'SDF_v1_64', synset, model_id)
                    
                    render_img_list = [os.path.join(render_img_dir, f) for f in os.listdir(render_img_dir) if ('.jpg' in f) and ('image' in f)]
                    ...
                    # for sdf
                    sdf_path = f'{dataroot}/FUTURE/SDF_v1_64/{synset}/{model_id}/ori_sample.h5'
                    sdf_list_s.append(sdf_path)

                    # for vox
                    #vox_path = os.path.join(dataroot, 'ShapeNet', 'ShapeNetVox32', synset, model_id, 'model.binvox')
                    vox_path = os.path.join(dataroot, 'FUTURE', 'SDF_v1_64', synset, model_id, 'model.binvox')
        ...
        if phase == 'train':
            ...
            self.transforms = transforms.Compose([
                ...
                #transforms.RandomCrop(200, pad_if_needed=False, padding_mode='padding_mode'),
            ])
        else:
            self.transforms = transforms.Compose([
        ...
        self.transforms_bg = transforms.Compose([
                transforms.RandomCrop(256, pad_if_needed=False, padding_mode='padding_mode'),
                transforms.Normalize(mean, std),
            ])

    def process_img(self, img, p):
        img_t = self.to_tensor(img)

        p_mask=p.replace('image', 'mask')
        mask_exists=os.path.exists(p_mask)
        #mask_exists=False
        if mask_exists:
            img=np.array(img)
            img_mask = np.array(Image.open(p_mask).convert('1')).astype(np.uint8) * 255
        else: 
            img=np.array(img)
            mask=np.ma.masked_less(img[:,:,2],190).data
            mask[mask>=190]=0
            mask[mask!=0]=255
            img_mask=mask
            #img_mask[img_mask==255]= 225
            #img_mask[img_mask==0]= 30
    ...
    def process_img_OOOLD(self, img, p, mode='noBG'):
        p_mask=p.replace('image', 'mask')
        mask_exists=os.path.exists(p_mask)
        ####
        mask_exists=False
    ...
    def __getitem__(self, index):
        ...
        sample_ixs=[0,1,0,0,0]
        print('len img all views ', imgs_all_view)
        for ix in sample_ixs:
            p = imgs_all_view[ix]
            im = Image.open(p).convert('RGB')
            im = self.process_img(im, p)
            imgs.append(im)
            img_paths.append(p)
        ...
        #print('obj path is ', model)
        #print('cat str is ', synset)
        ret = {
