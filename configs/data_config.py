...
## category mapping for autosdf object reconstruction training
cats= {
        "bed": "04530566",
        "bookshelf": "04090263",
        "cabinet": "03211117",
...
class Config(object):
    def __init__(self, dataset):
        ...
        if self.dataset == '3dfront':
            self.metadata_path = os.path.join(dataroot, self.dataset)
            self.train_test_data_path = os.path.join(self.metadata_path, '3dfront_train_test_data')
            self.__size_avg_path = os.path.join(self.metadata_path, 'preprocessed/new_size_avg_category.pkl')
            self.__layout_avg_file = os.path.join(self.metadata_path, 'preprocessed/avg_layout.pkl')
            self.bins = self.__initiate_bins()
            self.evaluation_path = './evaluation/3dfront'
    ...
    def __initiate_bins(self):
        ...
        if self.dataset == '3dfront':
            ...
            #bin['layout_centroid_avg'] = layout_avg_dict['avg_centroid']
            #bin['layout_coeffs_avg'] = layout_avg_dict['avg_size']
